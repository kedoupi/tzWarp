...
/// `/plan` from a locked terminal must enter the agent view with the full
/// prompt, not fall through to the shell.
#[test]
fn plan_slash_command_from_terminal_enters_agent_view_with_prompt() {
    App::test((), |mut app| async move {
        let _agent_view_flag = FeatureFlag::AgentView.override_enabled(true);
        initialize_app(&mut app);
        ...
        app.update(|ctx| {
            ctx.subscribe_to_view(&input, move |_, event: &super::Event, _| {
                if let super::Event::EnterAgentView { initial_prompt, .. } = event {
                    *captured_for_sub.borrow_mut() = initial_prompt.clone();
                }
            });
        });
...
#[test]
fn enter_on_plan_in_locked_terminal_does_not_fall_through_to_shell() {
    App::test((), |mut app| async move {
        let _agent_view_flag = FeatureFlag::AgentView.override_enabled(true);
        initialize_app(&mut app);
        ...
        input.update(&mut app, |input, ctx| {
            input.user_replace_editor_text("/plan 如果让你cr这个项目你应该怎么cr呢", ctx);
            input.input_enter(ctx);
        });

        assert_eq!(
            captured.borrow().as_deref(),
            Some("/plan 如果让你cr这个项目你应该怎么cr呢")
        );
    });
}

#[test]
fn queued_plan_slash_command_falls_through_to_prompt_submit() {
    App::test((), |mut app| async move {
        let _agent_view_flag = FeatureFlag::AgentView.override_enabled(true);
        initialize_app(&mut app);
