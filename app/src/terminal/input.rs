...
pub const INPUT_A11Y_HELPER: &str =
    "输入 shell 命令后按 Enter 执行。按 cmd-up 查看已执行命令的输出，按 cmd-l 重新聚焦命令输入。";
...
const AGENT_MODE_HINT_OPTIONS: &[&str] = &[
    "例如：部署 React 应用到 Vercel 并配置环境变量",
    "例如：排查 Python 测试在 CI 中失败的原因",
    "例如：用 Docker 新建微服务并创建部署流水线",
    "例如：查找并修复 Node.js 应用的内存泄漏",
    "例如：为 PostgreSQL 数据库创建定时备份脚本",
    "例如：将数据从 MySQL 迁移到 PostgreSQL",
    "例如：为 AWS 基础设施配置监控和告警",
    "例如：用 FastAPI 为移动应用构建 REST API",
    "例如：优化运行缓慢的 SQL 查询",
    "例如：创建 GitHub Actions 工作流，在合并后自动部署",
    "例如：为 Web 应用配置 Redis 缓存",
    "例如：排查 Kubernetes pod 持续崩溃的原因",
    "例如：构建处理 CSV 并导入 BigQuery 的数据管道",
    "例如：为域名配置 SSL 证书和 HTTPS",
    "例如：用现代设计模式重构旧代码",
    "例如：为认证服务创建单元测试",
    "例如：为分布式系统配置 ELK 日志聚合",
    "例如：在 Express.js 中实现 OAuth2 认证",
    "例如：优化 Docker 镜像以缩短构建时间并减小体积",
    "例如：为 Web 应用搭建 A/B 测试基础设施",
];
...
const AGENT_MODE_AI_ENABLED_STEER_HINT_TEXT_UDI: &str = "引导运行中的智能体";
const AGENT_MODE_AI_ENABLED_STEER_HINT_TEXT_CLASSIC: &str = "引导运行中的智能体，或按退格键退出";
const AGENT_MODE_AI_ENABLED_QUEUE_HINT_TEXT_UDI: &str = "为运行中的智能体排队后续消息";
const AGENT_MODE_AI_ENABLED_QUEUE_HINT_TEXT_CLASSIC: &str =
    "为运行中的智能体排队后续消息，或按退格键退出";
const AGENT_MODE_AI_ENABLED_FOLLOW_UP_HINT_TEXT_UDI: &str = "继续提问";
const AGENT_MODE_AI_ENABLED_FOLLOW_UP_HINT_TEXT_CLASSIC: &str = "继续提问，或按退格键退出";
...
const DYNAMIC_ENUM_GENERATE_MESSAGE: &str = "运行以下命令以生成选项：";
...
impl Input {
    ...
    fn subscribe_to_ambient_agent_view_model(
        ...
        ctx.subscribe_to_model(view_model, |me, handle, event, ctx| {
            ...
            if let AmbientAgentViewModelEvent::HandoffSnapshotUploadFailed { error_message } = event
            {
                ...
                let toast_message = format!("准备云端交接失败：{error_message}");
        ...
        match ai_query_routing {
            ...
            AIQueryRouting::LiveRemoteVm {
                ...
                if self.model.lock().shared_session_status().is_active_viewer() {
                    ...
                    self.show_ephemeral_error_toast("只读查看者无法发送查询。", ctx);
                } else {
            ...
            AIQueryRouting::NewCloudVm { task_id } => {
                if FeatureFlag::HandoffCloudCloud.is_enabled() {
                    ...
                    self.show_ephemeral_error_toast("此云端对话无法在本机继续。", ctx);
                }
                true
            }
    ...
    fn block_cloud_handoff_if_model_unsupported(&self, ctx: &mut ViewContext<Self>) -> bool {
        ...
        ToastStack::handle(ctx).update(ctx, |ts, ctx| {
            ts.add_ephemeral_toast(
                DismissibleToast::error(
                    "自定义模型无法在云端运行。请切换到 tzWarp 模型后交接。".to_owned(),
                ),
                window_id,
                ctx,
            );
    ...
    pub fn remove_excess_images(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if images_removed > 0 {
            ...
            let message = if images_removed == 1 {
                "已移除 1 张图片，每个对话最多 20 张。".into()
            } else {
                format!("已移除 {images_removed} 张图片，每个对话最多 20 张。")
            };
                    ...
                    && conversation.is_child_agent_conversation()
                {
                    ...
                    if conversation.status().is_in_progress() {
                        if is_queue_next_prompt_enabled {
                            return format!("为 {agent_name} 智能体排队后续消息");
                        }
                        return format!("引导 {agent_name} 智能体");
                    }
                    return format!("继续询问 {agent_name} 智能体");
                }
    ...
    fn cli_agent_rich_input_hint_text(&self, ctx: &ViewContext<Self>) -> Cow<'static, str> {
        ...
        CLIAgentSessionsModel::as_ref(ctx)
            ...
            .map(|session| match session.agent {
                ...
                _ => Cow::Owned(format!("向 {} 输入提示词...", session.agent.display_name())),
            })
        ...
        if self.prefix_mode(ctx) == InputPrefixMode::CloudHandoff {
            ...
            let hint = if conversation_is_empty {
                ...
                self.handoff_compose_state
                    ...
                    .map(|env| format!("交接给 {}", env.model().string_model.display_name()))
                    .unwrap_or_else(|| "交接到云端".to_owned())
            };
            self.editor.update(ctx, |editor, ctx| {
                editor.set_placeholder_text(&hint, ctx);
            });
    ...
    fn check_image_limits_for_paste(
        ...
        if excess_images > 0 {
            ...
            let message = if excess_images == 1 {
                format!("未附加 1 张图片，{limit_name}最多 {limit_value} 张。")
            } else {
                format!("未附加 {excess_images} 张图片，{limit_name}最多 {limit_value} 张。")
            };
                ...
                if FeatureFlag::AgentHarness.is_enabled() {
                    ...
                    if !availability.has_any_enabled_harness() {
                        ...
                        ToastStack::handle(ctx).update(ctx, |ts, ctx| {
                            ts.add_ephemeral_toast(
                                DismissibleToast::error(
                                    "没有可用的智能体运行环境。请联系团队管理员。".to_string(),
                                ),
                                window_id,
                                ctx,
                            );
        ...
        match action {
            ...
            InputAction::StartNewAgentConversation { origin } => {
                    ...
                    .can_start_new_conversation()
                {
                    ...
                    ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                        toast_stack.add_ephemeral_toast(
                            DismissibleToast::error("智能体监控命令时无法新建对话。".to_string()),
                            window_id,
                            ctx,
                        );
                    });
...
impl View for Input {
    ...
    fn keymap_context(&self, app: &AppContext) -> warpui::keymap::Context {
        ...
        if InputSettings::as_ref(app).slash_commands_available_in_terminal() {
