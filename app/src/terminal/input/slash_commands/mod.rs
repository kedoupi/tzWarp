...
impl Input {
    ...
    pub(super) fn execute_slash_command(
        ...
        match command.kind {
            ...
            SlashCommandKind::Compact | SlashCommandKind::Plan | SlashCommandKind::Orchestrate => {
                // Queued resubmit still falls through so `submit_queued_prompt` can send the
                // full `/plan …` text as a regular query on the existing conversation.
                if is_queued_prompt {
                    return false;
                }

                let prompt =
                    prompt_text_for_slash_command(command.name, argument.map(String::as_str));

                // Terminal input is locked to Shell when NLD is off (the default). Switching
                // to locked AI is rejected outside an agent view, so the old "return false
                // and hope Enter submits as AI" path sent `/plan` to the PTY.
                if FeatureFlag::AgentView.is_enabled()
    ...
    pub(super) fn maybe_handle_cmd_or_ctrl_shift_enter_for_slash_command(
        ...
        self.execute_detected_slash_or_skill(SlashCommandTrigger::cmd_or_ctrl_enter(), ctx)
    }
    ...
    pub(super) fn maybe_handle_enter_for_slash_command(
        ...
        self.execute_detected_slash_or_skill(SlashCommandTrigger::input(), ctx)
    }

    /// Executes the slash/skill command in the input, even if the live model was
    /// disabled (dismissed menu or slash-in-terminal setting off).
    fn execute_detected_slash_or_skill(
        &mut self,
        trigger: SlashCommandTrigger,
        ctx: &mut ViewContext<Self>,
    ) -> bool {
        let prompt = self.editor.as_ref(ctx).buffer_text(ctx);
        let state = match self.slash_command_model.as_ref(ctx).state() {
...
/// Reconstructs the buffer text a prompt-style slash command should submit.
pub fn prompt_text_for_slash_command(command_name: &str, argument: Option<&str>) -> String {
    match argument {
        Some(argument) if !argument.is_empty() => format!("{command_name} {argument}"),
        _ => command_name.to_owned(),
    }
}
