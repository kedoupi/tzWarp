use super::{prompt_text_for_slash_command, slash_command_is_submitted_as_prompt};
...
#[test]
fn prompt_text_for_slash_command_joins_name_and_argument() {
    assert_eq!(prompt_text_for_slash_command("/plan", None), "/plan");
    assert_eq!(prompt_text_for_slash_command("/plan", Some("")), "/plan");
    assert_eq!(
        prompt_text_for_slash_command("/plan", Some("审查这个项目")),
        "/plan 审查这个项目"
    );
    assert_eq!(
        prompt_text_for_slash_command("/orchestrate", Some("拆任务")),
        "/orchestrate 拆任务"
    );
}
