...
const DESCRIPTION_TEXT: &str = "添加 MCP 服务器以扩展智能体能力。MCP 以标准接口向智能体暴露数据源或工具，相当于插件。可添加自定义服务器，或从本机已有的 Claude / Codex 配置自动发现。";
...
const EMPTY_STATE_TEXT: &str = "添加 MCP 服务器后，会显示在这里。";
const NO_SEARCH_RESULTS_TEXT: &str = "没有找到搜索结果";
        ...
        > = std::sync::LazyLock::new(|| {
            vec![
                FormattedTextFragment::plain_text(
                    "自动从全局第三方智能体配置文件（例如用户主目录）检测并启动 MCP 服务器。仓库内检测到的服务器不会自动启动，需在下方「检测自」分区里单独开启。",
                ),
