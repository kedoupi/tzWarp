...
const NEXT_COMMAND_DESCRIPTION: &str =
    "让 AI 根据您的命令历史、输出结果和常见工作流程建议下一条命令。";
const PROMPT_SUGGESTIONS_DESCRIPTION: &str =
    "让 AI 根据最近的命令及其输出，在输入框中内联显示自然语言提示建议。";
const SUGGESTED_CODE_BANNERS_DESCRIPTION: &str =
    "让 AI 根据最近的命令及其输出，在块列表中内联显示代码差异和查询建议。";
...
const GIT_OPERATIONS_AUTOGEN_DESCRIPTION: &str = "让 AI 自动生成提交消息和拉取请求的标题及描述。";
...
impl AISettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let code_read_allowlist_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text("例如 ~/code/repo", ctx);
            input
        });
        Self::update_editor_interaction_state(
        ...
        let autodetection_denylist_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text("命令（逗号分隔）", ctx);
        ...
        let conversation_layout_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            match current {
                OpenConversationPreference::NewTab => {
                    dropdown.set_selected_by_name("新建标签页", ctx)
                }
                OpenConversationPreference::SplitPane => {
    ...
    fn start_grok_oauth(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            ...
            // never arrives.
            let toast =
                DismissibleToast::default("正在打开浏览器以连接 SuperGrok 订阅…".to_string())
    ...
    fn knowledge_widgets() -> Vec<Box<dyn SettingsWidget<View = Self>>> {
        ...
        widgets.push(Box::new(ManageRulesWidget::default()) as Box<dyn SettingsWidget<View = Self>>);
        #[cfg(not(feature = "team_relay"))]
        widgets.push(Box::new(WarpDriveContextWidget::default()));
        widgets
    }
    ...
    fn build_page(subpage: AISubpage, ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        match subpage {
            AISubpage::WarpAgent => {
                #[cfg(feature = "team_relay")]
                {
                    ...
                    widgets.push(Box::new(XiaotaoziTokenWidget::new(ctx)));
                }
                #[cfg(not(feature = "team_relay"))]
                {
        ...
        let title = match subpage {
            AISubpage::Knowledge => Some("知识库"),
            AISubpage::ThirdPartyCLIAgents => Some("第三方 CLI 代理"),
...
impl SettingsWidget for GlobalAIWidget {
    ...
    fn render(
        ...
        let is_anonymous = !cfg!(feature = "team_relay")
...
impl SettingsWidget for ActiveAIWidget {
    ...
    fn search_terms(&self) -> &str {
        "active ai a.i. next command prompt suggestions 主动AI 下一条命令 提示建议 代码建议 code diffs suggested banners passive unit tests commit pull request pr git code review autogen generate"
    }
...
impl AgentsWidget {
            ...
            .with_child(
                Container::new(
                    render_ai_setting_description(
                        "配置文件决定智能体怎么干活：能做哪些操作、何时要你确认、用哪个模型。也可以按项目单独指定。",
                        is_any_ai_enabled,
                        app,
                    )
                )
    ...
    fn render_base_model_setting(
        ...
        render_dropdown_item(
            ...
            "基础模型",
            Some(
                "此模型是智能体的主要引擎，负责多数交互，并在需要时调用其他模型进行规划或代码生成。也可能因可用性或摘要等辅助任务自动切换模型。",
            ),
    ...
    fn render_codebase_context_outline_generation_setting(
        ...
        let codebase_context_description = vec![
            FormattedTextFragment::plain_text(
                "允许智能体生成可用于上下文的代码库大纲。代码不会存储在我们的服务器上。",
            ),
    ...
    fn render_mcp_permissions_zero_state(
        ...
        let subtext = {
            let subtext_fragments = vec![
                FormattedTextFragment::plain_text(
                    "您尚未添加 MCP 服务器。添加后，即可控制智能体与其交互时的自主程度。",
                ),
    ...
    fn render(
        ...
        widget_children.push(
            Flex::column()
                ...
                .with_child(render_ai_setting_description(
                    "启用后，快进与自动批准可在不确认的情况下执行拒绝列表中的命令。",
                    is_any_ai_enabled,
                    app,
                ))
                .finish(),
        ...
        if FeatureFlag::QueueSlashCommand.is_enabled() {
            widget_children.push(render_dropdown_item(
                ...
                "默认提示提交模式",
                Some(
                    "当智能体仍在响应时提交新提示会如何处理。可在每个对话中用自动排队开关覆盖此设置。",
                ),
                None,
                LocalOnlyIconState::for_setting(
            ...
            if ai_settings.default_prompt_submission_mode == PromptSubmissionMode::Interrupt {
                widget_children.push(
                    Container::new(render_dropdown_item(
                        ...
                        "长时间运行命令的默认提交方式",
                        Some(
                            "智能体执行长时间运行命令时提交提示的处理方式。排队提示会在命令完成后发送。",
                        ),
                        None,
                        LocalOnlyIconState::for_setting(
    ...
    fn render_natural_language_detection_section(
        ...
        if FeatureFlag::AgentView.is_enabled() {
            static AUTODETECTION_DESCRIPTION_FRAGMENTS: LazyLock<Vec<FormattedTextFragment>> =
                LazyLock::new(|| {
                    vec![
                        FormattedTextFragment::plain_text("检测有误？"),
                        FormattedTextFragment::hyperlink(
                            "告诉我们",
...
impl SettingsWidget for WarpDriveContextWidget {
    ...
    fn render(
        ...
        let description = render_ai_setting_description(
            "智能体可利用 Warp Drive 内容，按个人与团队工作流定制回答，包括工作流、笔记本与环境变量。",
            ai_settings.is_any_ai_enabled(app),
            app,
        );
...
impl SettingsWidget for CLIAgentWidget {
    ...
    fn render(
        ...
        let description_fragments = vec![
            ...
            FormattedTextFragment::plain_text(" 等编码智能体时，显示快捷工具栏。"),
        ];
...
impl SettingsWidget for CLIAgentAutoToggleRichInputWidget {
    ...
    fn render(
        ...
        let label = render_body_item_label::<AISettingsPageAction>(
            "根据智能体状态自动显示/隐藏富输入".into(),
            Some(styles::header_font_color(true, app)),
            Some(AdditionalInfo {
                mouse_state: self.info_tooltip.clone(),
                on_click_action: None,
                secondary_text: None,
                tooltip_override_text: Some("需要对应编码智能体的 Warp 插件".to_owned()),
            }),
            LocalOnlyIconState::for_setting(
...
impl SettingsWidget for CLIAgentCommandsWidget {
    ...
    fn render(
        ...
        let description = appearance
            .ui_builder()
            .paragraph("添加正则：对匹配命令显示编码智能体工具栏。")
            .with_style(UiComponentStyles {
                font_size: Some(appearance.ui_font_size()),
        ...
        let toggle = if state.is_forced_by_org {
            ui_builder
                ...
                .with_tooltip(TooltipConfig {
                    text: "此选项由组织策略强制，无法自定义。".to_string(),
        ...
        if ai_settings.is_cloud_handoff_enabled(app) {
                ...
                .is_supported_on_current_platform()
            {
                ...
                column.add_child(render_ai_setting_description(
                    "当 macOS 即将休眠时，自动将最近聚焦的本地智能体对话转到云端模式以继续运行。",
                    true,
                    app,
                ));
            }
...
/// 小团队版：只展示一个「小桃子 API 密钥」输入框，不暴露多厂商 BYOK。
#[cfg(feature = "team_relay")]
#[derive(Clone)]
...
#[cfg(feature = "team_relay")]
struct XiaotaoziTokenWidget {
    editor: ViewHandle<EditorView>,
    /// Shared with async probe tasks; page re-renders via `ctx.notify()`.
    probe_status: std::sync::Arc<parking_lot::RwLock<RelayProbeStatus>>,
    signup_link: HighlightedHyperlink,
}

#[cfg(feature = "team_relay")]
impl XiaotaoziTokenWidget {
    fn new(ctx: &mut ViewContext<<Self as SettingsWidget>::View>) -> Self {
        let existing = crate::ai::team_relay::resolve_api_key(None);

        let probe_status = std::sync::Arc::new(parking_lot::RwLock::new(if existing.is_some() {
            RelayProbeStatus::Checking
        } else {
            RelayProbeStatus::NoKey
        }));

        let existing_for_editor = existing.clone();
        let editor = ctx.add_typed_action_view(move |ctx| {
            ...
            let mut editor = EditorView::single_line(options, ctx);
            editor.set_placeholder_text("粘贴小桃子 API 密钥", ctx);
            if let Some(key) = &existing_for_editor {
                editor.set_buffer_text(key, ctx);
            }
            editor
        });

        let probe_for_subscribe = probe_status.clone();
        ctx.subscribe_to_view(&editor, move |_, editor, event, ctx| {
            if matches!(event, EditorEvent::Blurred | EditorEvent::Enter) {
                ...
                if let Some(ref token) = key {
                    let _ = crate::ai::team_relay::write_token_to_disk(token);
                } else if let Some(home) = dirs::home_dir() {
                    let _ = std::fs::remove_file(home.join(".tzworp").join("token"));
                }
                Self::spawn_probe(key, probe_for_subscribe.clone(), ctx);
            }
        });
        ...
        ctx.subscribe_to_model(&Appearance::handle(ctx), move |_, _, event, ctx| {
            if let AppearanceEvent::ThemeChanged = event {
                ...
                editor_for_theme.update(ctx, move |editor, ctx| {
        ...
        // 首次进入设置时探测中转服务连通性。
        Self::spawn_probe(existing, probe_status.clone(), ctx);

        Self {
            editor,
            probe_status,
            signup_link: HighlightedHyperlink::default(),
        }
    }
    ...
    fn spawn_probe(
        ...
        let Some(api_key) = key.filter(|k| !k.trim().is_empty()) else {
            *probe_status.write() = RelayProbeStatus::NoKey;
            ctx.notify();
            return;
        };
        *probe_status.write() = RelayProbeStatus::Checking;
        ctx.notify();

        let status_for_task = probe_status;
        let base = crate::ai::team_relay::base_url();
        ctx.spawn(
            async move {
                crate::ai::team_relay::fetch_models(&api_key)
                    .await
                    .map(|models| models.len())
            },
            move |_me, result, ctx| {
                *status_for_task.write() = match result {
                    Ok(model_count) => RelayProbeStatus::Ok {
                        model_count,
                        base_url: base,
                    },
                    Err(e) => RelayProbeStatus::Err(e),
                };
                ctx.notify();
            },
        );
...
#[cfg(feature = "team_relay")]
impl SettingsWidget for XiaotaoziTokenWidget {
    type View = AISettingsPageView;

    fn search_terms(&self) -> &str {
        "小桃子 api key token 中转站 xiaotaozi tzworp activate 连通 申请 注册 账号"
    }
    ...
    fn render(
        ...
        let header_color = styles::header_font_color(is_enabled, app);
        let theme = appearance.theme();

        let title = Text::new_inline(
            "小桃子 API 密钥",
            appearance.ui_font_family(),
            PRIMARY_HEADER_FONT_SIZE * 0.55,
        )
        .with_style(Properties::default().weight(Weight::Bold))
        .with_color(header_color.into())
        .finish();

        let description_text = FormattedText::new([
            FormattedTextLine::Line(vec![
                FormattedTextFragment::plain_text("还没有密钥？先去小桃子"),
                FormattedTextFragment::hyperlink(
                    "申请账号",
                    crate::ai::team_relay::SIGNUP_URL,
                ),
                FormattedTextFragment::plain_text(
                    "，在控制台复制 API 密钥，粘贴到下面的输入框，回车保存。",
                ),
            ]),
            FormattedTextLine::LineBreak,
            FormattedTextLine::Line(vec![FormattedTextFragment::plain_text(
                "管理员也可以发激活链接 tzworp://activate?token=… 直接写入本机。请求只到小桃子，不经过 Warp。",
            )]),
        ]);
        let description = FormattedTextElement::new(
            description_text,
        ...
        let status_line = {
            let status = self.probe_status.read().clone();
            let (text, color) = match status {
                RelayProbeStatus::Checking => (
                    "正在检测中转服务连通性…".to_string(),
                    blended_colors::text_sub(theme, theme.surface_1()),
                ),
                RelayProbeStatus::NoKey => (
                    "未配置 API 密钥：请在设置 → 智能体填写小桃子 API 密钥，或设置环境变量 TZAI_API_KEY。"
                        .to_string(),
                ...
                RelayProbeStatus::Ok {
                    ...
                    format!("中转服务已连通 · {model_count} 个可用模型 · {base_url}"),
                    theme.ui_green_color(),
                ),
                RelayProbeStatus::Err(e) => {
                    (format!("中转服务连接失败：{e}"), theme.ui_error_color())
                }
            };
        ...
        let label = Text::new_inline("API 密钥", appearance.ui_font_family(), CONTENT_FONT_SIZE)
            .with_color(header_color.into())
            .finish();

        let field = Flex::column()
            .with_spacing(8.)
            .with_child(label)
            .with_child(input)
            .finish();
...
impl ApiKeysWidget {
    ...
    fn render_custom_inference_description(
        ...
        #[cfg(feature = "team_relay")]
        {
            let _ = (show_provider_keys, show_custom_endpoints, app);
            return Empty::new().finish();
        }
        ...
        if show_provider_keys {
            add_paragraph(vec![FormattedTextFragment::plain_text(
                "使用模型服务商的 API 密钥发起智能体请求。自动模型或未配置 API 密钥的模型会消耗 Warp 积分。",
            )]);
        }
        ...
        if show_custom_endpoints {
            add_paragraph(vec![FormattedTextFragment::plain_text(
                "添加自定义端点以使用第三方模型。自定义端点必须支持 OpenAI Chat Completions、OpenAI Responses 或 Anthropic Messages。",
            )]);
        }
        ...
        if show_provider_keys || show_custom_endpoints {
            add_paragraph(vec![FormattedTextFragment::plain_text(
                "API 密钥仅保存在本机安全存储，不会上传到 Warp 服务器。",
            )]);
    ...
    fn render_custom_inference_info_icon(
        ...
        let tooltip_text = if managed_byok_byoe_enabled {
            FormattedText::new([FormattedTextLine::Line(vec![
                FormattedTextFragment::plain_text("自定义推理设置由贵组织管理。"),
            ])])
        } else {
            FormattedText::new([FormattedTextLine::Line(vec![
                FormattedTextFragment::plain_text(
                    "使用自带密钥或自定义端点，即表示您同意仅按以下条款允许的方式使用：",
                ),
                FormattedTextFragment::hyperlink(
                ...
                FormattedTextFragment::plain_text(
                    "。自带密钥与自定义端点面向个人与小团队。超过 10 人的公司/组织应使用 Warp Business 或 Enterprise。",
                ),
            ])])
        };
        let tooltip_background = appearance.theme().tooltip_background();
    ...
    fn render_grok_subscription_row(
        ...
        let description = Container::new(
            Text::new(
                "连接 SuperGrok 订阅，以通过 xAI 账户在智能体中使用 Grok 模型。",
                appearance.ui_font_family(),
                CONTENT_FONT_SIZE,
            )
    ...
    fn render_warp_credit_fallback_toggle(
        ...
        let description = render_ai_setting_description(
            "启用后，出错时请求可能回退到 Warp 提供的模型。将优先使用您的 API 密钥，而非 Warp 积分。",
            ai_settings.is_any_ai_enabled(app),
            app,
        );
...
impl SettingsWidget for ApiKeysWidget {
    ...
    fn render(
        ...
        if visibility.show_section() {
            ...
            column.add_child(render_ai_setting_description(
                "贵组织统一管理自定义推理。个人 API 密钥与自定义端点当前已禁用。",
                is_any_ai_enabled,
                app,
            ));
        } else {
            // Fallback: old "API 密钥" header only
            column.add_child(
                build_sub_header(
                    appearance,
                    "API 密钥",
                ...
                UserWorkspaces::as_ref(app).team_for_view_handle(&self.view_handle, app)
            {
                if team.billing_metadata.customer_type == CustomerType::Enterprise {
                    vec![
                        ...
                        FormattedTextFragment::plain_text(" 以在企业计划上启用自带 API 密钥。"),
                    ]
                } else {
                    let current_user_email = auth_state.user_email().unwrap_or_default();
                    let has_admin_permissions = team.has_admin_permissions(&current_user_email);
                    ...
                    if has_admin_permissions {
                        vec![
                            ...
                            FormattedTextFragment::plain_text(" 以使用自己的 API 密钥。"),
                        ]
                    } else {
                        vec![FormattedTextFragment::plain_text(
                            "请联系团队管理员升级到 Build 计划以使用自有 API 密钥。",
                        )]
                    }
                }
...
impl AwsBedrockWidget {
    ...
    fn render_aws_bedrock_section(
        ...
        let toggle_description = if is_admin_enforced {
            ...
            "对支持 Bedrock 的模型，Warp 会加载并发送本地 AWS CLI 凭证。".to_string()
        };
