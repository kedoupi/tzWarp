    ...
    fn build_page(subpage: Option<AISubpage>, ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        match subpage {
            ...
            Some(AISubpage::WarpAgent) => {
                // tzWarp 小团队：总开关 + 仅小桃子 API 密钥（无多厂商）
        ...
        let title = match subpage {
            Some(AISubpage::Knowledge) => Some("知识库"),
            Some(AISubpage::ThirdPartyCLIAgents) => Some("第三方 CLI 代理"),
...
impl SettingsWidget for XiaotaoziTokenWidget {
    ...
    fn search_terms(&self) -> &str {
        "小桃子 api key token 中转站 xiaotaozi tzworp activate 连通"
    }
    ...
    fn render(
        ...
        let description_text = FormattedText::new([
            FormattedTextLine::Line(vec![FormattedTextFragment::plain_text(
                "仅接入小桃子中转服务。填写小桃子 API 密钥，或通过深链 tzworp://activate?token=… 自动写入。",
            )]),
            FormattedTextLine::LineBreak,
            FormattedTextLine::Line(vec![FormattedTextFragment::plain_text(
                "仅提供直接文本对话，不支持工具调用、项目上下文或其他厂商 API 密钥。请求仅发送至小桃子中转服务，不经过 Warp 官方服务。",
            )]),
        ]);
        let description = FormattedTextElement::new(
            description_text,
