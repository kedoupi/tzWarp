...
impl Display for SettingsSection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SettingsSection::WarpAgent => write!(f, "智能体设置"),
...
impl SettingsSection {
    ...
    /// Returns true if this section is a subpage under the "智能体" umbrella.
    ...
    /// Returns true if this section is a subpage under the "代码" umbrella.
    pub fn is_code_subpage(&self) -> bool {
        matches!(self, Self::CodeIndexing | Self::EditorAndCodeReview)
    }

    /// Returns true if this section is a subpage under the "云平台" umbrella.
...
impl FromStr for SettingsSection {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            // English deeplink/legacy + Chinese UI labels
            "About" | "关于" => Ok(Self::About),
            "Account" | "账户" | "账号" => Ok(Self::Account),
            "AI" => Ok(Self::AI),
            "MCP Servers" | "MCP 服务器" => Ok(Self::MCPServers),
            "Billing and usage" | "计费与用量" => Ok(Self::BillingAndUsage),
            "Appearance" | "外观" => Ok(Self::Appearance),
            "Code" | "代码" => Ok(Self::Code),
            "Features" | "功能" => Ok(Self::Features),
            "Keyboard shortcuts" | "键盘快捷键" => Ok(Self::Keybindings),
            "Privacy" | "隐私" => Ok(Self::Privacy),
            "Referrals" | "推荐" => Ok(Self::Referrals),
            "Scripting" | "脚本" => Ok(Self::Scripting),
            "Shared blocks" | "共享块" => Ok(Self::SharedBlocks),
            "Teams" | "团队" => Ok(Self::Teams),
            "Warpify" => Ok(Self::Warpify),
            "WarpDrive" | "Warp Drive" => Ok(Self::WarpDrive),
            // "Oz" legacy name for Warp Agent settings page
            "Oz" | "Warp Agent" | "Agent 设置" | "智能体" | "智能体设置" => {
                Ok(Self::WarpAgent)
            }
            "Profiles" | "AgentProfiles" | "配置文件" => Ok(Self::AgentProfiles),
            "MCP servers" | "AgentMCPServers" => Ok(Self::AgentMCPServers),
            "Knowledge" | "知识库" => Ok(Self::Knowledge),
            "Third party CLI agents" | "ThirdPartyCLIAgents" | "第三方 CLI 代理" => {
                Ok(Self::ThirdPartyCLIAgents)
            }
            "Indexing and projects" | "CodeIndexing" | "索引与项目" => Ok(Self::CodeIndexing),
            "Editor and Code Review" | "EditorAndCodeReview" | "编辑器与代码审查" => {
                Ok(Self::EditorAndCodeReview)
            }
            "CloudEnvironments" | "Environments" | "环境" => Ok(Self::CloudEnvironments),
            "Oz Cloud API Keys" | "OzCloudAPIKeys" | "Oz Cloud API 密钥" => {
...
impl SettingsView {
    pub fn new(page: Option<SettingsSection>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        // Build sidebar nav items. AI page is presented as an "智能体" umbrella
        // with subpages; the actual AI SettingsPage is hidden from direct sidebar listing.
        //
        // tzWarp 小团队模式：智能体子页（含配置文件 / MCP / 知识库 / 第三方 CLI）+ 外观 / 功能 / 快捷键 / 关于。
    ...
    pub fn set_and_refresh_current_page_internal(
        ...
        let section = match section {
            ...
            // 小团队模式不暴露 Warp 账户 / 推荐页，统一落到智能体设置。
            #[cfg(feature = "team_relay")]
            SettingsSection::Account | SettingsSection::Referrals => SettingsSection::WarpAgent,
            other => other,
        };
