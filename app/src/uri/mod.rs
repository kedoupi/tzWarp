...
pub fn handle_incoming_uri(url: &Url, ctx: &mut AppContext) {
    ...
    // tzWarp / 小桃子：tzworp://activate?token=... 激活中转服务 API 密钥
    #[cfg(feature = "team_relay")]
    if url.scheme() == "tzworp" {
        if let Some(token) = crate::ai::team_relay::parse_activate_url(url) {
            match crate::ai::team_relay::activate_token(&token, ctx) {
                Ok(()) => {
                    if let Some(window_id) = primary_window_id {
                        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                            let toast = DismissibleToast::success(
                                "小桃子 API 密钥已激活，可使用智能体".to_string(),
                            );
                            toast_stack.add_ephemeral_toast(toast, window_id, ctx);
                        });
                    }
                    log::info!(
                        "team_relay: activated token via deep link (len={})",
                        token.len()
                    );
                }
                Err(e) => {
                    if let Some(window_id) = primary_window_id {
                        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                            let toast = DismissibleToast::error(format!("API 密钥激活失败: {e}"));
                            toast_stack.add_ephemeral_toast(toast, window_id, ctx);
                        });
                    }
                    log::warn!("team_relay activate failed: {e}");
                }
            }
            return;
        }
        log::warn!("tzworp URI not recognized: {url}");
        return;
    }
