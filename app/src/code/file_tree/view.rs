...
const REMOTE_TEXT: &str = "项目资源管理器需要访问本机工作区，当前远程会话不支持。";
const DISABLED_TEXT: &str = "项目资源管理器需要访问本机工作区。请打开新会话，或切换到可用的会话后再查看。";
const WSL_TEXT: &str = "项目资源管理器目前无法在 WSL 中使用。";
            ...
            match item {
                FileTreeItem::File { .. } => {
                    ...
                    if !is_file_content_binary(&path_local) {
                        items.extend([
                            MenuItemFields::new("在新窗格打开")
                                .with_on_select_action(FileTreeAction::OpenInNewPane {
                                    id: id.clone(),
                                })
                                .into_item(),
                            MenuItemFields::new("在新标签页打开")
