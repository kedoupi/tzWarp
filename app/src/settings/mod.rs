...
impl CtrlTabBehavior {
    pub fn as_dropdown_label(&self) -> &str {
        match self {
            Self::ActivatePrevNextTab => "切换到上一个/下一个标签",
            Self::CycleMostRecentSession => "循环最近会话",
            Self::CycleMostRecentTab => "循环最近标签",
        }
    }
}
...
impl GlobalHotkeyMode {
    pub fn as_dropdown_label(&self) -> &str {
        match self {
            Self::Disabled => "关闭",
            Self::QuakeMode => "独立热键窗口",
            Self::ActivationHotkey => "显示/隐藏全部窗口",
        }
    }
}
