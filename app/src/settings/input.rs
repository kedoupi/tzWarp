...
impl InputSettings {
    ...
    /// Whether `/plan` and other slash commands are parsed in the terminal input.
    ///
    /// tzWarp always enables this: NLD stays off by default, so prompt-style
    /// commands must still be intercepted instead of being sent to the shell.
    pub fn slash_commands_available_in_terminal(&self) -> bool {
        cfg!(feature = "team_relay") || *self.enable_slash_commands_in_terminal.value()
    }
}
