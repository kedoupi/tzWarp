...
const DISABLE_IN_BAND_GENERATORS_MENU_ITEM_NAME: &str = "为新会话禁用带内生成器";
const ENABLE_PTY_RECORDING: &str = "启用 PTY 录制模式";
const DISABLE_PTY_RECORDING: &str = "禁用 PTY 录制模式";
...
const SHOW_SSH_COMMAND_BLOCKS_MENU_ITEM_NAME: &str = "显示 Warpified SSH 命令块";
const HIDE_SSH_COMMAND_BLOCKS_MENU_ITEM_NAME: &str = "隐藏 Warpified SSH 命令块";
const EXPORT_DEFAULT_SETTINGS_CSV_MENU_ITEM_NAME: &str = "导出默认设置为 CSV 到主目录";
...
fn make_new_app_menu(ctx: &AppContext) -> Menu {
    ...
    // 小团队模式使用 skip_login 假用户，不提供 Warp「退出登录」。
    #[cfg(not(feature = "team_relay"))]
    {
        menu_items.push(MenuItem::Separator);
        menu_items.push(MenuItem::Custom(CustomMenuItem::new(
...
fn make_new_help_menu() -> Menu {
    Menu::new(
        ...
        vec![
            feedback_menu_item(),
            link_menu_item("文档…", links::USER_DOCS_URL.into()),
...
fn make_new_elements_menu_items(ctx: &AppContext) -> Vec<MenuItem> {
    ...
    // matches the user's "新会话的默认模式" setting. The non-default item
