...
impl AIExecutionProfilesModel {
    ...
    fn should_wait_for_cloud_profile_import(ctx: &AppContext) -> bool {
        // tzWarp 不连 Warp Drive，云端首包永远不会完成；等它会让「添加配置」静默失败。
        if cfg!(feature = "team_relay") {
            return false;
        }
        let auth = AuthStateProvider::as_ref(ctx).get();
        if auth.is_anonymous_or_logged_out() {
            return false;
        }
        auth.user_id().is_some() && !UpdateManager::as_ref(ctx).has_completed_initial_load()
    }
    ...
    fn activate_pending_settings_collection(
        ...
        if Self::should_wait_for_cloud_profile_import(ctx) {
            return false;
        }

        if !cfg!(feature = "team_relay") && Self::cloud_collection_awaiting_reconciliation(ctx) {
            return false;
        }
    ...
    fn new_profile_display_name() -> String {
        if cfg!(feature = "team_relay") {
            "未命名".into()
        } else {
            String::new()
    ...
    pub fn create_profile(&mut self, ctx: &mut ModelContext<Self>) -> Option<ExecutionProfileId> {
        ...
        if self.settings_migration_state == SettingsMigrationState::PendingLegacyImport {
            ...
            new_profile.name = Self::new_profile_display_name();
        ...
        if self.settings_are_authoritative() || cfg!(feature = "team_relay") {
            let mut profiles = if self.settings_are_authoritative() {
                AISettings::as_ref(ctx).execution_profiles.value().clone()
            } else {
                self.pending_legacy_profiles(ctx)
            };
            let mut new_profile = self.default_profile(ctx).data().clone();
            new_profile.name = Self::new_profile_display_name();
            new_profile.is_default_profile = false;
