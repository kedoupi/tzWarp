...
pub const WORKSPACE_OVERRIDE_TOOLTIP_MESSAGE: &str = "此项由组织策略强制，无法单独修改。";
...
pub fn render_models_section(
    ...
    let mut column = Flex::column()
        ...
        .with_child(render_filterable_dropdown_row(
            appearance,
            "基础模型",
            "智能体的主模型，负责大多数对话，并在需要时调用其他模型做规划或写代码。模型不可用或做摘要这类辅助任务时，可能会自动换模型。",
    ...
    column = column.with_child(render_filterable_dropdown_row(
        appearance,
        "完整终端模型",
        "智能体进入交互式终端（数据库 shell、调试器、REPL、开发服务器）时使用的模型：读取实时输出并向 PTY 写入命令。",
...
fn render_context_window_row(
    ...
    let desc = Text::new(
        "基础模型一次能纳入的对话、代码和文档量。窗口越大，长对话和大仓库越连贯，但延迟和消耗也更高。".to_string(),
        appearance.ui_font_family(),
        11.,
    )
    .with_color(
...
pub fn render_plan_auto_sync_toggle(
    ...
    let desc_elem = Text::new(
        "智能体创建的计划会自动保存并同步到 Warp Drive。".to_string(),
        appearance.ui_font_family(),
        11.,
    )
    .with_color(
