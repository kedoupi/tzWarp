    ...
    fn default() -> Self {
        Self {
            agent_mode: AvailableLLMs {
                ...
                choices: vec![LLMInfo {
                    display_name: "自动（省额度）".to_owned(),
                    base_model_name: "自动（省额度）".to_owned(),
...
impl LLMPreferences {
    ...
    pub fn refresh_available_models(&self, ctx: &mut ModelContext<Self>) {
        ...
        {
            self.refresh_team_relay_models(ctx);
            return;
        }
    ...
    /// tzWarp：从团队中转站拉取模型列表，填充 Agent 模型选择器。
    #[cfg(feature = "team_relay")]
    fn refresh_team_relay_models(&self, ctx: &mut ModelContext<Self>) {
        use crate::ai::team_relay;

        let Some(api_key) = team_relay::resolve_api_key(None) else {
            // 无 key 时提供默认本地占位模型，避免选择器全空
            let update = team_relay_fallback_models();
            if update != self.models_by_feature {
                // apply via spawn callback path
                let update_clone = update.clone();
                ctx.spawn(
                    async move { Ok::<_, anyhow::Error>(update_clone) },
                    |me, result, ctx| {
        ...
        ctx.spawn(
            async move {
                let models = team_relay::fetch_models(&api_key)
                    .await
                    .map_err(|e| anyhow::anyhow!(e))?;
                Ok::<ModelsByFeature, anyhow::Error>(team_relay_models_to_feature(models))
            },
            |me, result: Result<ModelsByFeature, anyhow::Error>, ctx| match result {
                Ok(update) => {
                    if update != me.models_by_feature {
                        me.on_server_update(update, ctx);
                    } else {
                ...
                Err(e) => {
                    report_error!(e.context("Failed to fetch team relay LLMs"));
                    me.set_agent_mode_models_unavailable(true);
                }
            },
        );
...
#[cfg(feature = "team_relay")]
fn team_relay_fallback_models() -> ModelsByFeature {
    use crate::ai::team_relay;

    let info = LLMInfo {
        display_name: format!(
            "{} ({})",
            team_relay::DEFAULT_MODEL,
            team_relay::RELAY_DISPLAY_NAME
        ),
        base_model_name: team_relay::DEFAULT_MODEL.to_owned(),
        id: team_relay::DEFAULT_MODEL.to_owned().into(),
        reasoning_level: None,
        usage_metadata: LLMUsageMetadata {
            request_multiplier: 1,
            credit_multiplier: None,
        },
        description: Some(team_relay::RELAY_DISPLAY_NAME.to_owned()),
        disable_reason: None,
        vision_supported: true,
        spec: None,
    ...
    let available = AvailableLLMs {
        default_id: team_relay::DEFAULT_MODEL.to_owned().into(),
        choices: vec![info],
        preferred_codex_model_id: None,
    };
    ModelsByFeature {
...
#[cfg(feature = "team_relay")]
fn team_relay_models_to_feature(models: Vec<crate::ai::team_relay::ModelInfo>) -> ModelsByFeature {
    use crate::ai::team_relay;

    let choices: Vec<LLMInfo> = models
        .into_iter()
        .map(|m| {
            let label = m.label();
            LLMInfo {
                display_name: label.clone(),
                base_model_name: m.id.clone(),
                id: m.id.clone().into(),
                reasoning_level: None,
                usage_metadata: LLMUsageMetadata {
                    request_multiplier: 1,
                ...
                description: Some(format!("{} · {}", team_relay::RELAY_DISPLAY_NAME, m.id)),
                disable_reason: None,
                vision_supported: true,
                spec: None,
                provider: LLMProvider::OpenAI,
    ...
    let default_id = if choices
        .iter()
        .any(|c| c.id.as_str() == team_relay::DEFAULT_MODEL)
    {
        team_relay::DEFAULT_MODEL.to_owned().into()
    } else {
        choices
            .first()
            .map(|c| c.id.clone())
            .unwrap_or_else(|| team_relay::DEFAULT_MODEL.to_owned().into())
    };
