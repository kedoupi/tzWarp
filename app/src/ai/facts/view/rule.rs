...
const DESCRIPTION_TEXT: &str = "规则会作为结构化指引提供给智能体，帮助保持风格一致、落实最佳实践，并适配特定工作流（包括代码库或更广泛的任务）。";

const SEARCH_PLACEHOLDER_TEXT: &str = "搜索规则";
const ZERO_STATE_TEXT: &str =
    "在上方添加一条规则，或把规则放到 ~/.agents/AGENTS.md，即可应用到所有项目。";
const ZERO_STATE_TEXT_PROJECT: &str =
    "为项目生成 WARP.md 规则文件后，它会显示在这里。";

const DISABLED_BANNER_TEXT: &str =
    "规则当前已关闭，不会作为会话上下文使用。你可以随时";
const DISABLED_BANNER_LINK_TEXT: &str = "重新开启";
