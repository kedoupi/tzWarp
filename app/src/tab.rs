...
impl TabData {
    ...
    fn copy_metadata_menu_items(
        ...
        if !uses_vertical_tabs(ctx) {
            Self::push_copy_metadata_menu_item(&mut menu_items, "复制标签标题", tab_title);
            return menu_items;
        }
        ...
        let (title_label, title, terminal_view) = if matches!(
            ...
            ("复制标签标题", tab_title, terminal_view)
        };
    ...
    fn pin_menu_items(&self, index: usize) -> Vec<MenuItem<WorkspaceAction>> {
        ...
        let (label, action) = if self.pinned {
            ("取消固定标签", WorkspaceAction::UnpinTab(index))
        } else {
            ("固定标签", WorkspaceAction::PinTab(index))
        };
        vec![
            MenuItemFields::new(label)
