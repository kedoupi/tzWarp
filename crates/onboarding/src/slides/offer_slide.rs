...
impl OfferVariant {
    pub(crate) fn title(self) -> &'static str {
        match self {
            OfferVariant::HeadStart => "开始使用 AI",
            OfferVariant::ChooseHowToStart => "选择使用方式",
        }
    }
    ...
    pub(crate) fn primary_badge_label(self, pricing_promotion_message: Option<&str>) -> &str {
        match self {
            OfferVariant::HeadStart | OfferVariant::ChooseHowToStart => {
                pricing_promotion_message.unwrap_or("推荐")
            }
        }
    }
    ...
    pub(crate) fn subtitle(self) -> Option<&'static str> {
        match self {
            OfferVariant::HeadStart => Some("可按当前环境配置 AI 功能。"),
            OfferVariant::ChooseHowToStart => Some("可稍后在设置中配置 AI。"),
        }
    }

    pub(crate) fn primary_label(self) -> &'static str {
        match self {
            OfferVariant::HeadStart => "配置更多 AI 功能",
            // The plan and the one-time credit packs now share one card, so it
            // is titled for their common outcome: using Warp with AI.
            OfferVariant::ChooseHowToStart => "使用 tzWarp AI",
        }
    }
    ...
    pub(crate) fn primary_description(self, shows_credit_packs: bool) -> &'static str {
        match self {
            OfferVariant::HeadStart => "查看当前环境可用的 AI 功能。",
            OfferVariant::ChooseHowToStart if shows_credit_packs => {
                "tzWarp Agent 可配合本地和已配置的模型使用。服务与价格以实际可用内容为准。"
            }
            OfferVariant::ChooseHowToStart => "tzWarp Agent 可协助处理终端任务、代码修改和验证。",
        }
    }

    pub(crate) fn secondary_label(self) -> &'static str {
        match self {
            OfferVariant::HeadStart => "稍后配置 AI",
            OfferVariant::ChooseHowToStart => "稍后配置 AI",
        }
    }

    pub(crate) fn secondary_description(self) -> &'static str {
        match self {
            OfferVariant::HeadStart => "先使用终端，后续可在设置中配置 AI。",
            OfferVariant::ChooseHowToStart => {
...
impl OfferSlide {
    ...
    fn render_credit_pack_tile(
        ...
        let mut badge = Container::new(
            appearance
                .ui_builder()
                .paragraph(format!("省 {badge_percent}%"))
                .with_style(UiComponentStyles {
                    font_size: Some(11.),
    ...
    fn render_purchase_status(
        ...
        Some(
            appearance
                .ui_builder()
                .paragraph("无法开始购买。请重试，或选择“稍后配置 AI”继续。")
                .with_style(UiComponentStyles {
                    font_size: Some(13.),
