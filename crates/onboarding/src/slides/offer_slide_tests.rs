...
const CONTROL_PRIMARY_LABEL: &str = "使用 tzWarp AI";
const CONTROL_PRIMARY_DESCRIPTION: &str = "tzWarp Agent 可协助处理终端任务、代码修改和验证。";
...
fn head_start_copy_and_telemetry_names_match_spec() {
    ...
    assert_eq!(variant.title(), "开始使用 AI");
    assert_eq!(variant.subtitle(), Some("可按当前环境配置 AI 功能。"));
    assert_eq!(variant.primary_label(), "配置更多 AI 功能");
    assert_eq!(
        variant.primary_description(false),
        "查看当前环境可用的 AI 功能。"
    );
    assert_eq!(variant.secondary_label(), "稍后配置 AI");
    assert_eq!(
...
fn choose_how_to_start_copy_and_telemetry_names_match_spec() {
    ...
    assert_eq!(variant.title(), "选择使用方式");
    assert_eq!(variant.subtitle(), Some("可稍后在设置中配置 AI。"));
    assert_eq!(variant.primary_label(), "使用 tzWarp AI");
    assert_eq!(variant.subscribe_label(), "查看可用服务");
    assert_eq!(
        variant.primary_description(true),
        "tzWarp Agent 可配合本地和已配置的模型使用。服务与价格以实际可用内容为准。"
    );
    assert_eq!(variant.secondary_label(), "稍后配置 AI");
    assert_eq!(
        variant.secondary_description(),
        "使用终端、已配置模型或其他 CLI Agent。后续可在设置中配置 AI。"
    );
...
fn promotion_replaces_recommended_on_both_offer_variants() {
    ...
    assert_eq!(OfferVariant::HeadStart.primary_badge_label(None), "推荐");
}
...
fn subscribe_copy_frames_add_on_credits_as_a_saving() {
    ...
    assert!(description.contains("服务与价格以实际可用内容为准"));
}
...
fn subscribe_copy_drops_the_add_on_line_when_no_packs_are_shown() {
    ...
    assert_eq!(
        without_packs,
        "tzWarp Agent 可协助处理终端任务、代码修改和验证。"
    );
    assert!(!without_packs.contains("价格"));
