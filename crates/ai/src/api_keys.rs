...
impl ApiKeyManager {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        // tzWarp / team relay: seed OpenAI-compatible key from env if storage empty.
        if keys.openai.as_deref().unwrap_or("").trim().is_empty() {
            for var in ["TEAM_RELAY_API_KEY", "TZAI_API_KEY"] {
                if let Ok(v) = std::env::var(var) {
                    let t = v.trim();
                    if !t.is_empty() {
                        keys.openai = Some(t.to_string());
                        break;
                    }
                }
            }
